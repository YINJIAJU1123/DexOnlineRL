...
try:
    ...
    if not rclpy.ok():
        rclpy.init()
    builtins.GLOBAL_ROS_EXECUTOR = SingleThreadedExecutor()
    print(f"[Actor.py] 全局 Executor 创建成功！对象: {builtins.GLOBAL_ROS_EXECUTOR}")
    print(">>> 提示: 后续代码将直接复用此对象，不再尝试新建。")
...
def on_press(key):
    try:
        ...
        if key == keyboard.Key.space:
            print("\n>>> [键盘控制] 按下 'Space': 确认继续 / 下一步")
            shared_state.terminate = True  # base_env 里等待的就是这个信号
            time.sleep(0.2)
            return
        ...
        if hasattr(key, 'char') and key.char is not None:
            # === 按键 A: 切换人工干预 ===
            if key.char == 'a':
                new_state = not shared_state.human_intervention_key
                shared_state.human_intervention_key = new_state
                print(f"\n>>> [键盘控制] 按下 'A': 人工干预已切换为 -> {new_state}")
                time.sleep(0.2)
...
# 启动监听器
try:
    print(">>> 键盘监听已启动: [A]=干预开关, [B]=停止程序")
    listener = keyboard.Listener(on_press=on_press)
    listener.start()
except Exception as e:
    print("Error starting keyboard listener:", e)
    sys.exit(1)
...
def actor_cli(env_cfg):
    ...
    elif "frankaleap" in env_cfg.robot_config.robot_type:       # by Jiaju
        lerobot_config_path = "../../train_config_silri_frankaleap.json"
    else:
...
def make_policy_obs(obs: dict, device: torch.device, robot_type: str) -> dict:
    # # =======================================================   by Jiaju
    # print("\n" + "!"*50)
    # print(f"  [DEBUG] make_policy_obs 收到 obs，类型: {type(obs)}")
    # print(f"  [DEBUG] 第一层 Keys: {list(obs.keys())}")
    # if "images" in obs:
    #     print(f"  [DEBUG] obs['images'] 类型: {type(obs['images'])}")
    #     if isinstance(obs['images'], dict):
    #         # 看看里面有没有 cam_chest
    #         print(f"  [DEBUG] obs['images'] 内部 Keys: {list(obs['images'].keys())}")
    #         # 顺便看看数据类型，是不是 numpy 数组
    #         for k, v in obs['images'].items():
    #             print(f"   -> Image '{k}': Type={type(v)}")
    #             if hasattr(v, 'shape'):
    #                 print(f"      Shape={v.shape}")
    # else:
    #     print("  [DEBUG] ⚠️ 警告：obs 里没有 'images' 键！")
    
    # if "state" in obs:
    #     print(f"  [DEBUG] obs['state'] 类型: {type(obs['state'])}")
    #     if isinstance(obs['state'], dict):
    #          print(f"  [DEBUG] obs['state'] 内部 Keys: {list(obs['state'].keys())}")
