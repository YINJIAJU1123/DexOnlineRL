...
def train_cli(env_cfg):
    if env_cfg.robot_config.robot_type == "ur_wrist":
        ...
        #env_cfg.lerobot_config_path = "/home/yin/Online_RL/HIL-RL/train_config_silri_franka.json"
        env_cfg.lerobot_config_path = "/home/lixin/OnlineRl/train_config_silri_franka.json"
...
def offline_training(cfg: TrainRLServerPipelineConfig, policy: nn.Module, optimizers: dict, offline_replay_buffer: ReplayBuffer, wandb_logger: WandBLogger | None):
    ...
    for optimization_step in tqdm.tqdm(range(500), desc="Offline training"):
        ...
        def fix_images_in_dict(obs_dict):      # by Jiaju
            new_dict = {}
            for k, v in obs_dict.items():
                # 先把原来数据拷过来
                val = v
                # 判断是否为图像 (维度是4: Batch, Channel, H, W) 且 key 里包含 'images'
                if "images" in k and v.ndim == 4:
                    # 1. Center Crop: 640x480 -> 480x480
                    if v.shape[-1] == 640 and v.shape[-2] == 480:
                        start_x = (640 - 480) // 2
                        val = val[..., :, start_x : start_x + 480]
        ...
        observations = fix_images_in_dict(observations)        # by Jiaju
        next_observations = fix_images_in_dict(next_observations)    # by Jiaju
